      ...
      <div>
        ...
        {selected && (
          <p className="text-xs text-muted-foreground mt-1">
            Market price: <span className="font-semibold text-foreground">৳{marketPrice.toFixed(2)}</span> per share ·
            Bought at: <span className="font-semibold text-foreground">৳{Number(selected.purchasePrice).toFixed(2)}</span>
          </p>
        )}
      </div>
        ...
        {price && Number(price) > 0 && (
          <div className="mt-1.5 flex items-center justify-between text-xs">
            <span className="text-muted-foreground">
              Total listing value: <span className="font-bold text-foreground">৳{total.toFixed(2)}</span>
            </span>
