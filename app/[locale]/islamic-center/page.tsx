...
export default async function IslamicCenterPage() {
  const [stats, t] = await Promise.all([getStats(), getTranslations("islamicCenter")]);
  ...
  const SECTIONS = [
    {
      key: "quran",
      titleKey: "sectionQuranTitle" as const,
      descKey: "sectionQuranDesc" as const,
      icon: "📖",
    ...
    {
      key: "duas",
      titleKey: "sectionDuasTitle" as const,
      descKey: "sectionDuasDesc" as const,
      icon: "🤲",
    ...
    {
      key: "articles",
      titleKey: "sectionArticlesTitle" as const,
      descKey: "sectionArticlesDesc" as const,
      icon: "📝",
    ...
    {
      key: "pdfs",
      titleKey: "sectionPdfsTitle" as const,
      descKey: "sectionPdfsDesc" as const,
      icon: "📄",
    ...
    <div className="min-h-screen bg-muted">
      ...
      <div className="bg-white border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="mt-5 inline-flex items-center gap-2.5 bg-green-50 border border-green-200 rounded-xl px-4 py-2.5">
            ...
            <div>
              <p className="text-xs font-semibold text-green-800">{t("prayerTimesNote")}</p>
              <p className="text-xs text-green-700">{t("prayerTimesVisit")}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
            ...
            <Link key={s.key} href={s.href}>
              <div className="bg-white rounded-2xl border border-border overflow-hidden hover:shadow-md transition-shadow group h-full flex flex-col">
                <div className={`bg-gradient-to-br ${s.color} px-5 pt-6 pb-5`}>
                  ...
                  <h3 className="font-bold text-foreground group-hover:text-brand transition-colors">{t(s.titleKey)}</h3>
                </div>
                <div className="px-5 py-4 flex-1">
                  ...
                  <p className="text-xs font-semibold text-brand mt-3">
                    {counts[s.key] > 0 ? t("countAvailable", { count: counts[s.key] }) : t("comingSoon")}
                  </p>
                </div>
              </div>
            </Link>
        ...
        {stats.recentDuas.length > 0 && (
          <div className="bg-white rounded-2xl border border-border overflow-hidden">
            <div className="px-6 py-4 border-b border-border flex items-center justify-between">
              <h2 className="font-semibold text-foreground">{t("featuredDuas")}</h2>
              <Link href="/islamic-center/duas" className="text-xs text-brand font-medium hover:underline">{t("viewAll")}</Link>
            </div>
            <div className="divide-y divide-border">
              {stats.recentDuas.map((dua) => (
                <div key={dua.id} className="px-6 py-5">
                  ...
                  {dua.source && (
                    <p className="text-xs text-muted-foreground mt-1">{t("source")}: {dua.source}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {stats.recentArticles.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-semibold text-foreground">{t("recentArticles")}</h2>
              <Link href="/islamic-center/articles" className="text-xs text-brand font-medium hover:underline">{t("viewAll")}</Link>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {stats.recentArticles.map((a) => (
                <div key={a.id} className="bg-white rounded-xl border border-border p-5">
                  ...
                  <p className="text-xs text-muted-foreground mt-3">{t("by")} {a.author.fullName}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {stats.recentPdfs.length > 0 && (
          <div className="bg-white rounded-2xl border border-border overflow-hidden">
            <div className="px-6 py-4 border-b border-border flex items-center justify-between">
              <h2 className="font-semibold text-foreground">{t("pdfLibrary")}</h2>
              <Link href="/islamic-center/pdfs" className="text-xs text-brand font-medium hover:underline">{t("browseAll")}</Link>
            </div>
        ...
        {stats.surahCount === 0 && stats.duaCount === 0 && stats.articleCount === 0 && stats.pdfCount === 0 && (
          <div className="bg-white rounded-2xl border border-border px-8 py-12 text-center">
            ...
            <p className="font-semibold text-foreground mb-2">{t("contentComingSoon")}</p>
            <p className="text-sm text-muted-foreground max-w-md mx-auto">
              {t("contentComingSoonDesc")}
            </p>
          </div>
        )}
      </div>
