...
import { requestAirTicketAction } from "@/app/actions/air-ticket";
import Link from "next/link";

export function AirTicketRequestForm() {
  const [state, action, pending] = useActionState(requestAirTicketAction, null);
  ...
  return (
    <div className="bg-white rounded-2xl border border-border overflow-hidden">
      <div className="px-7 py-5 border-b border-border">
        <h2 className="font-bold text-foreground text-lg">Request a Flight</h2>
        <p className="text-xs text-muted-foreground mt-0.5">Fill in your trip details and we&apos;ll confirm within 1 hour</p>
      </div>
      ...
      <form action={action} className="p-7 space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">Departure date</label>
            <input
              name="departDate"
              type="date"
              required
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">
              Return date <span className="text-muted-foreground font-normal">(optional, leave blank for one-way)</span>
            </label>
            <input
              name="returnDate"
              type="date"
          ...
          <div className="sm:col-span-2">
            ...
            <textarea
              name="notes"
              rows={3}
              placeholder="Preferred airline, seat preference, luggage needs, etc."
        ...
        >
          {pending ? "Submitting Request…" : "Request Flight"}
        </button>
      </form>
    </div>
  );
